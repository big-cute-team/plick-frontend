...
다만 이번 PR에서는 커서를 실제로 쓰지는 않았다. `getArticles`가 `cursor`를 받고 `nextCursor`를 돌려주는
데까지만 깔아 두고, 무한스크롤은 넣지 않았다. 홈 소식 리스트는 최신 10건을 보여주는 자리고, 무한스크롤이
정말 필요한 표면은 릴스다. 엔드포인트 하나가 작업 하나라는 원칙대로 PR을 작게 유지했고, 배관은 이미
깔려 있어서 나중에 `useInfiniteQuery`로 바꾸는 건 훅 하나 갈아끼우는 일이 된다.
