...
### 새 글은 언제 보이나, 그리고 왜 30페이지를 다 받나

포커스 재요청을 끄고 나니 다음 질문이 따라왔다. 새 글이 자주 올라오는 서비스인데 지금 설정으로 그게
언제 화면에 보이느냐는 것이었다.

먼저 정리해 둘 오해가 하나 있다. **staleTime이 지났다고 RQ가 알아서 다시 받아오지 않는다.** stale은
"다음에 트리거가 오면 새로 받아라"라는 표시일 뿐이다. 폴링도 없고 소켓도 없으니 홈을 열어 두고 가만히
있으면 새 글이 백 개 올라와도 화면은 그대로다. 갱신이 걸리는 건 하드 리로드, 홈으로 다시 들어오기
(`refetchOnMount`), 네트워크 재연결뿐이다.

그다음 나온 질문이 더 날카로웠다. 왜 1페이지만 새로 받고 이어가면 되지 않고 30페이지를 다 받느냐는
것이었다. 답은 그렇게 하면 목록에 구멍이 나기 때문이다.

새 글 N1, N2가 들어온 상황을 놓고 보면 이렇다.

```
갱신 전   1페이지 = [A1..A10]      nextCursor = cursor(A10)
          2페이지 = [A11..A20]  ← 저장된 pageParam = cursor(A10)

1페이지만 다시 받으면 = [N1, N2, A1..A8]   nextCursor = cursor(A8)
2페이지는 저장된 cursor(A10)으로 받은 [A11..A20] 그대로

이어붙이면  ... A8 | A11 ...        A9와 A10이 조용히 사라진다
```

그래서 RQ는 저장해 둔 커서를 재사용하지 않고 매번 새 응답에서 다시 뽑는다. 앞에 인용한 루프에서
0페이지만 `oldPageParams[0]`을 쓰고 나머지는 `getNextPageParam(options, result)`으로 재도출하는 게
그 이유다. 체인을 처음부터 다시 엮는 것이고, 커서 체인이라 병렬로도 못 한다.

남는 선택지는 둘뿐이다. 전부 다시 체이닝해서 보던 300건을 유지하거나(요청 N개), 2페이지 이후를 버리고
1페이지만 받거나(요청 1개, 화면의 300건이 10건으로 줄어듦). RQ가 전자를 기본값으로 삼은 건 사용자가
지금 250번째 항목을 보고 있는지 알 방법이 없어서다. 보는 중에 목록이 줄면 발밑이 꺼진다.

### 그런데 우리 홈에서는 전자가 낭비다

RQ가 페이지를 지키는 전제는 사용자가 그 깊은 위치를 계속 보고 있다는 것이다. 우리 홈은 그렇지 않다.
릴스에 갔다 오면 `NewsFeed`가 언마운트됐다가 새로 마운트되고, DOM이 통째로 다시 만들어지니
`ScrollArea`의 `scrollTop`은 0이다. Next의 스크롤 복원은 document 스크롤이 대상인데 우리 스크롤 주체는
중첩된 `overflow-y-auto` 컨테이너라 복원 대상이 아니다. 즉 어차피 맨 위로 돌아온다. 보지도 않을
29페이지를 정합성 맞추겠다고 다시 받는 셈이다.

(이건 DOM 동작으로 추론한 것이고 브라우저로 확인하지는 못했다.)

### gcTime을 staleTime에 붙였다

그래서 `gcTime`을 기본값 5분에서 `staleTime`과 같은 60초로 내렸다. 이 둘의 간극이 비싼 구간을 만들고
있었다.

```
전:  60초 이내 → 캐시 그대로 (요청 0)
     60초~5분  → N페이지 재체이닝 (요청 N)   ← 비싼 구간
     5분 이후  → 엔트리 소멸, initialData 사용 (요청 0)

후:  60초 이내 → 캐시 그대로 (요청 0)
     60초 이후 → 엔트리 소멸, initialData 사용 (요청 0)
```

가운데가 사라진다. 그리고 여기서 그동안 버려지던 게 살아난다. 홈에 올 때마다 서버 컴포넌트는 이미
`getArticles()`로 첫 페이지를 새로 받고 있었는데, 캐시 엔트리가 있으면 그 결과가 무시된다고 앞에
적었다. `gcTime`을 내리면 엔트리가 없는 상태로 돌아오게 되니 그 서버 fetch가 실제로 쓰인다. 결과적으로
어느 경우든 클라 요청 0개에 항상 최신이다. 대가는 1페이지로 리셋되는 것뿐이고 스크롤도 어차피 맨
위다.

두 값이 갈라지면 비싼 구간이 조용히 되살아나므로 `FEED_FRESH_MS` 상수 하나로 묶어 두고 주석에 이유를
적었다. 인입 주기가 몇 분에 한두 개라 1분이면 사실상 매번 최신을 본다.

### 묵은 씨앗이 신선한 척하는 문제

`gcTime`을 내리니 앞서 지나쳤던 함정이 실제로 물게 됐다. `initial` prop은 서버 렌더 시점에 고정된
값이다. 홈에 머물면서 팀 탭을 오가면 ALL 쿼리가 관찰자를 잃고 60초 뒤 정리되는데, ALL로 돌아올 때
엔트리를 새로 만들면서 그 `initial`이 다시 씨앗으로 들어간다. 화면에 20분 머물렀다면 20분 묵은
데이터다.

더 나쁜 건 `initialData`가 `dataUpdatedAt`을 재적용 시점의 `Date.now()`로 찍는다는 점이다.
20분 묵은 데이터가 방금 받은 것으로 취급돼 60초간 갱신조차 안 걸린다.

그래서 서버가 응답을 받은 시각을 함께 넘기고 `initialDataUpdatedAt`에 물렸다. 페이지와 시각을
`InitialArticleFeed`라는 한 덩어리로 묶어서, 데이터만 넘기고 시각을 빠뜨리는 실수를 타입으로 막았다.
이제 묵은 씨앗은 심자마자 stale로 판정돼 마운트 갱신이 걸린다. 그 시점엔 페이지가 하나뿐이라 요청도
하나다.

여기에 시계 문제가 하나 더 붙는다. `dataUpdatedAt`은 클라이언트의 `Date.now()`와 비교되는데 서버가 찍은
시각이라 기기 시계가 틀어져 있으면 계산이 어긋난다. 기기가 뒤처져 있으면 서버 시각이 미래로 보여
영영 신선한 것이 되므로, 지금보다 미래인 값은 지금으로 깎았다.
...
반대 방향(기기 시계가 앞선 경우)은 실제보다 낡게 보여 갱신이 한 번 더 걸릴 뿐이라 안전한 쪽으로
틀어진다. 함수로 넘긴 건 RQ가 캐시 엔트리를 만들 때만 호출해서, 렌더마다 `Date.now()`를 부르지 않게
하려는 것이다.

앞으로 이게 부족해지면(홈 소식을 깊게 스크롤하는 사용이 실제로 많아지면) `maxPages`로 캐시에 유지할
페이지 수에 상한을 두는 레버가 남아 있다. 지금 넣을 이유는 없다.
