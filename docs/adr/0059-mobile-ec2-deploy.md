# 0059. 웹·모바일 EC2 배포와 CD 파이프라인 (KAN-334)

백엔드가 전부 AWS에 올라갔고, 프론트용 EC2도 하나 받았다. 이제 Next 앱을 그 위에 띄우고
main에 병합되면 자동으로 배포되게 만드는 게 이번 작업이다.

처음에는 모바일만 올릴 생각이었는데 작업 도중에 범위가 늘었다. 같은 인스턴스에 web(3000)과
mobile(3001)을 함께 올리고 `plick.co.kr`과 `m.plick.co.kr`로 가르기로 했다. 그 얘기는
아래 "두 앱을 한 인스턴스에" 절에 몰아 적는다. 앞부분은 모바일 하나로 파이프라인을 세우면서
막혔던 기록이고, 그 결론은 web에도 그대로 적용됐다.
...
여기까지 만들어 놓고 보니 web도 같은 EC2에 올리는 게 자연스러웠다. 인스턴스를 t3.small
(2GB)로 잡아둔 게 여기서 값을 한다. Next standalone 프로세스가 유휴 시 150~250MB, 부하를
받으면 300~500MB쯤 쓰니 두 개를 띄워도 여유가 있다. 1GB짜리였으면 nginx도 없는 마당에
빠듯했을 것이다.

배포 시점도 같이 바꿨다. 원래 develop 푸시에 걸어뒀는데, CLAUDE.md에 이미 "main은
릴리스용"이라고 못박혀 있다. develop에 머지될 때마다 실서비스가 재기동되는 건 그 원칙과
안 맞는다. main 푸시로 옮겼다. develop에서 main으로 병합하는 건 사람이 직접 하므로,
그 병합이 곧 릴리스 결정이 된다.

앱마다 다른 건 포트와 도메인뿐이라 격리는 이름으로만 갈랐다.
...
`mobile-release.sh`는 `release.sh`로 이름을 바꾸고 `APP` 환경변수를 받게 고쳤다. 안에서
`ROOT=/srv/plick-$APP`, `PM2_NAME=plick-$APP`으로 조립하니 분기가 하나도 안 생겼다.
`PORT`는 각 앱의 `shared/.env`에서 오므로 헬스체크도 자연히 자기 포트를 본다.

워크플로는 매트릭스로 돌린다. 두 앱이 같은 인스턴스에 뜨지만 디렉터리도 pm2 프로세스도 따로라
동시에 나가도 부딪히지 않는다. `fail-fast: false`를 준 이유는, 한쪽 배포가 실패했을 때
다른 쪽 잡이 중간에 취소되면 그쪽 롤백까지 못 끝내고 죽기 때문이다.

### apex 도메인이 걸린다

`m.plick.co.kr`은 서브도메인이라 CNAME으로 ALB를 가리키면 끝인데, `plick.co.kr`은 그게 안 된다.
DNS 규격상 도메인 꼭대기(apex, zone apex)에는 CNAME을 둘 수 없다. apex에는 SOA와 NS 레코드가
반드시 있어야 하는데 CNAME은 다른 레코드와 공존하지 못한다는 규칙 때문이다.

ALB는 고정 IP가 없어서 A 레코드에 IP를 박는 것도 안 된다. 결국 둘 중 하나다.

- 네임서버를 Route 53으로 옮기고 A 레코드 Alias로 ALB를 지정한다. Alias는 AWS가 만든
  확장이라 apex에도 걸 수 있고 요금도 없다
- 지금 쓰는 DNS 업체가 ALIAS나 ANAME 같은 CNAME 평탄화 기능을 지원하면 그걸 쓴다

Route 53으로 옮기는 쪽이 확실하다. 호스팅 영역 하나에 월 0.5달러 수준이고 ALB Alias가
정석 조합이다. 이건 코드로 풀 수 있는 문제가 아니라 DNS 설정 작업이다.

### 인증서는 하나로 묶는다

ACM 인증서 하나에 `plick.co.kr`과 `m.plick.co.kr`을 둘 다 넣는다. 대체 이름(SAN)으로
여러 도메인을 한 인증서에 담을 수 있어서 리스너에 인증서를 두 번 붙일 필요가 없다.
`*.plick.co.kr` 와일드카드로 받아도 되는데, 와일드카드는 apex(`plick.co.kr`)를 포함하지
않으므로 그 경우에도 apex를 별도 이름으로 추가해야 한다.

ALB 리스너 443에는 기본 동작 대신 호스트 헤더 규칙 두 개를 건다. `plick.co.kr`이면
web 대상 그룹(3000), `m.plick.co.kr`이면 mobile 대상 그룹(3001)으로 보낸다. 대상 그룹의
헬스체크 경로는 둘 다 `/api/health`다.
...
- BE 앞단 프라이빗 ALB가 아직 안 만들어졌다. 그게 생기면 `API_BASE_URL` 시크릿에 그 내부 ALB의
  ...
  예고 없이 바꾼다
- EC2에 web 쪽 자리가 아직 없다. `/srv/plick-web/{releases,shared}`를 만들고
  `shared/.env`에 `PORT=3000`과 `OAUTH_REDIRECT_URI=https://plick.co.kr/oauth/callback`을
  넣어야 첫 배포가 돈다. mobile 쪽은 이미 만들어 뒀다
- `plick.co.kr` apex를 ALB로 보내려면 Route 53으로 네임서버를 옮겨야 한다. 위에 쓴 CNAME
  제약 때문이고, 이번 작업에서 가장 먼저 손봐야 할 외부 의존이다
- 프론트 ALB, ACM 인증서(두 도메인 SAN), 호스트 기반 리스너 규칙 두 개
- 카카오·구글 콘솔에 `https://plick.co.kr/oauth/callback`과
  `https://m.plick.co.kr/oauth/callback` 둘 다 등록. 포트만 다를 뿐 별개 URI라 각각 필요하다
  (ADR 0023 전례)
- ALB가 붙기 전까지는 3000·3001을 임시로 열어 `http://<EIP>:3001` 식으로 확인할 수 있다.
  이 경로에서는 위에 쓴 이유로 로그인이 안 되는 게 정상이다. 확인이 끝나면 두 포트 모두
  ALB 보안그룹에서만 들어오도록 좁힌다
- 배포 워크플로와 CI(`ci.yml`)가 따로 돈다. main 푸시 시 두 워크플로가 동시에 시작하므로,
  타입 에러가 있는 커밋도 일단 배포는 나간다. develop에서 이미 CI를 통과한 커밋만 main으로
  가는 구조라 위험이 크지는 않지만, 나중에 `workflow_run`으로 CI 성공에 물리는 걸 검토한다
- 모바일 앱이 Vercel에도 연결돼 있다. EC2가 정본이 되면 어느 쪽을 남길지 정해야 한다
