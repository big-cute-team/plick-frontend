...
- 콘솔 작업은 사용자 몫이다. 롤에 인라인 정책 둘(`grafana-cloudwatch-read`,
  `monitoring-ssm-read`), SSM 파라미터, 슬랙 Incoming Webhook. 순서는 절차서 3.1과 3.1.1.
- 이미 떠 있는 모니터링 EC2에는 6절 갱신 절차로 프로비저닝 파일과 `.env`를 넣고 그라파나를
  다시 만든다.
- 앱 쪽 변경(보고 라우트, 카운터)은 main 병합 후 prod 배포가 나가야 산다.
