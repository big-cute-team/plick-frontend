...
### 로딩이 느리다 — 근접 릴만 생성 + preconnect

임베드 로딩이 너무 느리다는 피드백이 왔다. 절반은 구조적이다. 공식 임베드는 트윗 하나가
작은 웹페이지 하나다 — widgets.js 로드, iframe 생성, iframe 안에서 X의 JS 번들과 폰트 로드,
데이터 fetch, 렌더 후 높이 postMessage까지가 한 세트다. react-tweet이 JSON 하나 받아 바로
그리던 것과 비교하면 느릴 수밖에 없고, 이건 전환의 트레이드오프다.

나머지 절반은 우리 문제였다. 릴스가 슬라이드 전 장을 마운트하면서 임베드 10개를 동시에
생성하고 있었다. iframe 10개가 대역폭을 나눠 먹고, X의 익명 세션 rate limit(짧은 시간 다수
요청 시 빈 위젯)도 더 잘 걸린다. 그래서 보고 있는 릴 앞뒤 한 장까지만 위젯을 생성하게 했다.
ReelsFeed가 이미 들고 있는 activeIndex로 `near`(±1) prop을 내려보내고, ReelItem은 near가 한
번이라도 켜지면 그때부터 임베드를 유지한다 — 지나갔다고 없앴다 다시 만들면 재로드가 더
비싸다. 렌더 중 상태 보정 패턴(`useState` 초기값 + 렌더 중 조건부 set)으로 effect 없이 처리했다.
검증에서 첫 로드에 위젯이 2개(현재+다음)만 생기고, 스와이프마다 다음 장이 이어 생성되며
지나온 장은 유지되는 걸 확인했다.

연결 비용도 줄였다. 루트 레이아웃 head에 `platform.twitter.com` preconnect를 걸어 DNS·TLS
수립을 미리 끝내고, widgets.js 자체도 릴스 피드 마운트 시점에 미리 받게 했다
(`preloadTweetWidgets` — 로더가 모듈 스코프 싱글턴 Promise라 중복 로드는 없다).
