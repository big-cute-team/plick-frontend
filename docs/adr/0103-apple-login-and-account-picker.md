...
## 약관·개인정보처리방침·FAQ 문안 개정

버튼만 달고 끝날 일이 아니었다. 이용약관과 개인정보처리방침이 가입 수단을 "카카오·구글"로
못박아 두고 있었다. 법률 문서가 실제 서비스와 어긋나는 상태가 되는 거라 같은 PR에서 개정했다.
`PrivacyPolicyBody` 파일 머리에 "수집 항목이 바뀌면 이 문서도 같은 PR에서 개정한다"는 경고가
이미 박혀 있었는데, 이번이 딱 그 케이스였다.

문안 단일 출처는 `@plick/ui`의 `TermsBody`·`PrivacyPolicyBody`·`FaqBody`다(법률 문서라
web/mobile 문안이 어긋나면 안 되어 공용화돼 있다). 고친 곳:

- 이용약관 제2조(정의)·제4조(이용계약 체결): 가입 수단에 애플 추가. 제4조 2항의 "14세
  미만의 단독 가입을 제한하는" 문구는 애플에도 성립하는지 확인했다 — 한국 기준 Apple ID
  단독 생성은 14세 이상이고, 미만은 가족 공유로 보호자가 만들어야 해서 '단독 가입 제한'
  구조가 카카오·구글과 같다.
- 개인정보처리방침 1항(처리 목적)·3항(14세 미만): 프로바이더 나열에 애플 추가.
- 개인정보처리방침 2항(처리 항목): 프로바이더에 애플을 추가하면서 이메일 주소에 "제공자와
  계정 상태에 따라 제공되지 않을 수 있습니다"를 명시했다. 우리 애플 인가 요청은 scope를
  안 보내서 이메일을 아예 못 받고(식별자 sub만), 카카오도 원래 이메일이 없는 계정이 있다.
  기존 문안은 이메일을 무조건 받는 것처럼 읽혀서 이참에 실제 처리 현황에 맞췄다.
- FAQ "로그인하지 않아도 쓸 수 있나요?" 답변: 지원 프로바이더에 애플 추가.
- `@plick/domain/brand`의 로그인·회원가입 페이지 SEO description: "카카오·구글" →
  "카카오·구글·애플". 네이버 권장 80자 이내는 여전히 지킨다(회원가입이 79자로 아슬아슬).
- 이메일 부재를 설명하던 코드 주석들(`profile.ts`, `me/page.tsx`, `InfoCard`,
  `@plick/domain/types`)도 "카카오 가입이면" → "카카오·애플 가입이면"으로 맞췄다.
  화면 로직은 이미 이메일 null을 처리하고 있어서 코드 변경은 없다.

시행일이 문제였다. 약관 제3조가 "개정 시 적용일 7일 전부터 공지"를 스스로 요구한다.
오늘(8. 24.) 게시 기준으로 7일 뒤인 2026. 8. 31.을 시행일로 잡고, 부칙과 방침 14항의
직전 버전 정보(8. 19.)와 변경 내용을 갱신했다. 애플 로그인 기능 자체는 그 전에 배포돼도
가입 수단이 늘어나는 유리한 변경이라 실무상 문제는 없다고 봤다.
