...
컨테이너를 다시 만들자 프론트 잡 4개(2대 × mobile·web)가 `down`으로 잡혔고, 사용자가 PR을
develop과 main에 병합해 prod 배포가 나간 뒤 그라파나에서 새 인스턴스 2대가 `up`으로 바뀌는 걸
봤다. Blue/Green이 인스턴스 ID를 통째로 바꿨는데도 Name 태그 필터가 그대로 따라왔다. 아래
그래프에는 web의 BE 호출과 path별 p95가 실제 트래픽으로 그려졌다. 404는 크롤러가 없는 기사
ID를 두드린 흔적이었다.

노트북 쪽은 awscli와 session-manager-plugin을 brew로 깔고 IAM 사용자 액세스 키를 넣었다.
"키를 꼭 만들어야 하나"는 질문이 있었는데, 포트 포워딩은 노트북의 CLI가 AWS에 직접 여는
연결이라 브라우저 콘솔 세션으로는 안 되고, 키 없이 가려면 IAM Identity Center(SSO)를 계정에
켜야 해서 BE 팀과 같이 쓰는 계정 설정을 건드리게 된다. 지금은 키를 좁은 권한으로 만들고
나중에 SSO로 갈아타면 지우기로 했다.

상단 타깃 패널이 6시간 범위의 마지막 값을 보여 줘서 교체된 옛 인스턴스 0이 몇 시간 남았다.
쿼리를 instant로 바꿔 지금 살아 있는 타깃만 보이게 했다.

배포 워크플로는 인스턴스가 `up`이 된 뒤에도 한참 `in_progress`였다. CodeDeploy Blue/Green이
트래픽을 넘긴 뒤 옛 인스턴스를 종료하기까지 대기 시간을 두기 때문이고, 배포 자체는 그 전에
끝난 것이다.
