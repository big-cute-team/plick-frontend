...
위 stage 값은 처음 `#dfe4ec`(media-bg)→다음 `#ffffff`로 손봤지만, **다크에서도 stage(#0b101d)와
bg(#0b0d12)가 눈에 띄게 다르다**는 재제보가 이어졌다. 결국 문제의 본질은 **`media-stage`가 bg와
별개 토큰이라 테마마다 따로 관리해야 하고 계속 어긋난다**는 것 → 토큰을 **제거**하고 릴 뷰어를
`bg-bg`로 바꿔 뷰어·바디·패널이 항상 같은 배경이 되게 했다(라이트·다크 모두 이음새 0).

단, `media-stage`가 다크로 유지됐던 원래 이유는 **카드 밖 데스크톱 액션 레일이 `media/on`(흰색)이라
밝은 배경에선 안 보이기 때문**이었다. 그래서 뷰어를 bg로 바꾸는 것과 함께 `ReelActionRail`에
`tone`을 추가했다 — `media`(사진 위 오버레이=모바일, 흰색 그대로)와 `surface`(페이지 배경 위=데스크톱
카드 밖, `text-text`·`bg-elevate` 테마색). 데스크톱 레일만 `surface`로 넘겨 라이트(어두운 아이콘)·
다크(밝은 아이콘) 양쪽에서 보이게 했다. 모바일 오버레이 레일은 기본값 `media`라 동작 불변.
