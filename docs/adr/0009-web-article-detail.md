...
위 stage 값은 처음 `#dfe4ec`(media-bg)로 뒀는데, **세부 패널(흰색)이 열리면 회색 뷰어 vs 흰 패널이
어긋나 보인다**는 재제보를 받아 `#ffffff`(페이지 bg)로 바로잡았다. media-bg(#dfe4ec)는 미디어
placeholder 회색이라 레터박스에 부적합 — 다크에서 stage(#0b101d)가 bg(#0b0d12)와 사실상 구분
안 되는 관계를 라이트에서도 지키려면 stage는 media-bg가 아니라 **bg**를 따라가야 한다.
