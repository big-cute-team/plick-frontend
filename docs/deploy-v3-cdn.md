...
진행 상황은 §16 표 기준으로 9번(체크리스트)까지 왔다. 컷오버까지 마친 뒤 이
환경 전체를 개발용으로 확정했다. 실서비스는 같은 구성을 VPC 하나 더 만들어
복제할 계획이다. 그래서 접근 잠금은 보안그룹 `/32`(컷오버 전 방식)가 아니라
CloudFront 앞 WAF의 팀 IP 허용 목록으로 옮겼다. §9의 보안그룹 절반(CloudFront
접두사 목록 허용)은 §8 검증 단계로 앞당겨 했다. 남은 것은 §9의 헤더 검증과
80 정리, §10의 관리형 규칙(WebACL 자체는 잠금 용도로 먼저 생겼다), 수명
주기(§12), standalone 복사 제거 판단(§16 14번)이다. 시행착오 전체는
[ADR 0082](adr/0082-cdn-static-split-groundwork.md)에 있다.
...
⚠️ 콘솔 생성 흐름이 6단계 마법사(Get started → Specify origin → Enable
security → Get TLS certificate → Review)로 바뀌었다. 마법사는 오리진 하나와
기본 비헤이비어까지만 받고, S3 오리진과 정적 비헤이비어는 생성 뒤 원본·동작
탭에서 추가한다. 마법사에서 기본값을 그대로 받으면 안 되는 곳이 셋이다.

- Get started의 Domain(Route 53 managed domain) 칸은 비워 둔다. 채우면
  alias·인증서·레코드를 마법사가 한꺼번에 붙여 §8의 컷오버 순서가 무너진다
- Cache settings의 권장값 `UseOriginCacheControlHeaders`를 받지 않는다.
  오리진 헤더를 따라 캐시하는 정책이라 오리진의 헤더 실수가 곧바로 HTML·RSC
  캐시 사고가 된다. Customize를 골라 아래 값을 직접 준다
- Enable security는 비활성화를 고른다. 켜면 배포마다 Block 모드 WebACL이
  생기고 과금이 시작된다. WAF는 §10에서 하나 만들어 두 배포에 붙인다
...
alias를 붙일 때 같이 정하는 값 둘. 보안 정책은 기본 선택인 TLSv1.3 전용이 아니라
`TLSv1.2_2021(권장)`로 둔다. 1.3 전용은 TLS 1.2까지만 지원하는 구형 모바일
브라우저를 끊는다. HTTP/3는 켠다. 미지원 브라우저는 HTTP/2로 알아서 내려간다.
...
2. §6의 배포 2개를 alias 없이 만든다. 상태가 Deployed가 될 때까지 기다린다
   (5~15분)
3. `dxxxxxxxx.cloudfront.net`으로 직접 열어 검증한다. 이때 브라우저
   ...
   ⚠️ HTML은 이 단계에서 404(`server: awselb/2.0`)가 나오는 게 정상이다.
   계획을 처음 쓸 때 놓친 지점인데, 뷰어 Host가 `dxxx.cloudfront.net`이고
   `AllViewer`가 그걸 그대로 ALB에 넘기므로 호스트 규칙에 안 맞아 리스너
   기본 작업으로 떨어진다. 그 404 자체가 연결·오리진 TLS·Host 전달까지의
   증명이라 스모크 테스트로는 충분하다. HTML 렌더까지 미리 보려면 ALB
   리스너에 CloudFront 기본 도메인 2개를 각 대상 그룹으로 보내는 임시 규칙을
   달았다가 검증 후 지운다. 실제 작업에서는 임시 규칙 없이 404 확인까지만
   하고 컷오버로 넘어가 실도메인으로 전체를 검증했다
...
이 절반은 이미 했다. 오픈 전 잠금(인바운드가 팀 IP `/32`뿐)에 CloudFront가
막혀 504가 나서, 접두사 목록 허용을 §8 검증 단계에서 앞당겨 추가했다.
`0.0.0.0/0`을 좁힌 게 아니라 원래 좁던 곳에 추가한 것이라 롤백선 문제는
없었다. `/32` 규칙은 디버깅용 직통 경로로 남겨 뒀다.

원래 계획: 퍼블릭 ALB 보안그룹의 인바운드 443 규칙에서 소스를 `0.0.0.0/0`에서
관리형 접두사 목록 `com.amazonaws.global.cloudfront.origin-facing`으로 바꾼다.
⚠️ 목록이 두 개 뜬다. `ipv6.cloudfront` 쪽이 아니라 무접두사
`cloudfront.origin-facing`(pl-22a6434b)이다. 오리진 설정이 IPv4-only라서다.
...
- §9 후반: 리스너 규칙에 `X-Origin-Verify` 헤더 조건 추가, 80 리스너 정리.
  헤더를 심어만 두고 검증을 안 켠 지금은 남의 CloudFront 배포가
  `origin.plick.co.kr`을 오리진 삼아 우회할 수 있는 구멍이 열려 있다
- WAF 부착, Count → Block (§10)
- S3 수명 주기 90일 (§12)
- 다음 배포에서 §0 재현 검증: 배포 전 탭의 소프트 내비게이션이 404 안 나는지,
  이전 세대 청크 URL이 200인지. 첫 배포 시점엔 세대가 하나뿐이라 못 밟았다
- `/_next/image*` 전용 캐시 비헤이비어 (§6)
- `public/` 자산 캐시 비헤이비어 (§6)
- 뷰어 IPv6용 AAAA 별칭 추가 검토 (지금은 A만 있다)
