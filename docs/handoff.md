...
- **완료(PoC)**: 릴스 2패널(Embla), 로그인/온보딩/마이페이지 화면, 디자인 토큰(t27), 빅6 팀색, 제스처/바운스/seam 이슈 해결, 문서(토큰·컴포넌트·데이터·라우팅·모노레포).
- **모노레포**: 부트스트랩 완료, `@plick/tokens` 이관 중.
- **다음**: core/ui 패키지 이관 → PoC `app/embla/*` → `apps/mobile` 라우트로 이관(비교군 motion/swiper/scroll-snap 폐기) → 실제 라우팅(`/reels/[postId]`) → API 연동.

## 14. 미해결 질문

- 닉네임 출처(OAuth vs 별도 입력), best/live 정렬 기준, 마이팀 피드 정의, 페이지네이션(cursor 확정), OG 썸네일 생성 방식, 공유 카운트/딥링크 범위, 전역 상태 라이브러리 도입 여부, catalog 버전 고정 적용, 네이티브 셸 Capacitor 확정.
