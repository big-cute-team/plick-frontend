...
> 2, 3, 4는 KAN-350·KAN-351에서 끝났다([ADR 0072](adr/0072-team-hub-url-filter.md),
> [ADR 0073](adr/0073-jsonld-dynamic-og.md)). 5(시맨틱 마크업)와 6(캐시)은 KAN-380에서
> 처리했다([ADR 0086](adr/0086-seo-followups.md)). 1(`/reels/[postId]`)만 남았다.
...
> KAN-380에서 5의 측정 수단(GA4)을 붙였다([ADR 0086](adr/0086-seo-followups.md)).
> 개인정보처리방침도 같이 개정했다. 나머지(1 서비스 소개, 2 thin content, 3 CWV,
> 4 백링크, 5의 운영 사이클)는 배포 후 또는 별도 티켓이다.
>
> 전략 문서에 없던 항목 하나가 KAN-380에서 추가로 처리됐다. 환경 분리(KAN-377) 이후
> dev 도메인이 prod와 같은 콘텐츠를 색인 허용 상태로 서비스하고 있어서, dev 빌드에
> noindex를 붙이고 사이트맵 안내를 뺐다. manifest도 이때 채웠다.
