...
`_lib/types.ts`로 경계에서 변환하고, BE 필드명이 화면 관용과 다르면 데이터 레이어에서 흡수한다.
...
(`apps/mobile/app/_lib/api/users.ts`). 만료는 미들웨어가 refresh로 잇는다(ADR 0021).
...
퍼블리싱 때는 각 앱 `_lib/mock.ts`가 화면에 데이터를 먹였고 타입은 `_lib/types.ts`가 BE 목표 shape로 들고 있었다.
...
3. 데이터 접근 레이어를 작성한다. `apiFetch` 위에 도메인 fetcher를 `_lib`에 두고 경계 변환도 여기서 한다.
...
| fetch 인프라 (`apiFetch` 래퍼, RQ provider와 쿼리키 규약)        | 첫 앱은 앱별 `_lib`에 둔다. 두 번째 앱이 같은 규약을 쓰면 `@plick/core` 승격 검토.              | 데이터나 타입과 같은 성숙도 게이트를 탄다.                                        |
...
A. 앱 역참조 금지. 패키지가 `@/_lib/…`나 라우트, `TEAMS`를 내부에서 조회하면 실격이다. 못 끊으면 승격할 수 없다.
...
`import { HOT_POSTS } from "@/_lib/mock"`이 `const posts = await getHotPosts()`가 된다.
