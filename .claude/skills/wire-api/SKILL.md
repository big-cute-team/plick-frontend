---
name: wire-api
description: 로컬 BE(스웨거) API 하나를 화면에 연결(mock→fetch)하고, 공용화 여부를 ADR 0011 기준으로 판단해 PR 제목·본문을 채팅에 건넨다
...
모바일 연결이 끝난 API를 web에 이식하는 작업이면 이 스킬이 아니라 `/web-wire-api`를 쓴다.
...
1. 계약 확인은 `be-verify` 서브에이전트에 위임한다. 공개 GET이든 보호 API든 직접 curl과 psql을 두들기지 않는다.
   ...
   다 밟고 `scripts/be-verify/report.schema.json` 형태의 JSON 리포트 하나를 돌려준다.
   티켓 설명도 스웨거 설명도 믿지 않는다. 리포트의 `swaggerDiffs`가 실제 계약이다(ADR 0025, 0026에서 실제로 틀렸다).
   `blocked`가 null이 아니면 거기서 멈추고 사용자에게 알린다.
2. 계획 확인. 리포트를 바탕으로 타입 변경, 데이터 레이어 배치, 페칭 도구, 공용화 후보를 정리해 한 번 보여주고 진행한다.
   페이지네이션 계약이나 revalidate 전략처럼 리포트로도 안 정해지는 건 여기서 사용자에게 묻는다.
3. 타입 대조. 응답 shape를 기존 `_types/`와 맞추고, 다르면 타입을 조정하되 web과 mobile을 같은 모양으로
   동기화한다. 필드명 차이는 데이터 레이어 경계 변환으로 흡수한다.
4. 브랜치. `git switch -c feature/<티켓>-<짧은설명> develop`.
5. 데이터 레이어 작성이나 교체. `apiFetch` 위에 도메인 fetcher를 `_services/`에 두고 경계 변환도 거기서 한다.
   화면은 소비 형태를 유지한 채 mock 대신 그 결과를 받는다. env와 프록시 세팅은 `data-layer.md` §1.
6. 페칭 도구 선택. 단발 읽기면 서버 컴포넌트 fetch, 릴스 페이지네이션이나 뮤테이션이면 TanStack Query
   (`tanstack-query.md`의 도입 트리거, provider, infinite, 낙관적 뮤테이션, 하이드레이션). 로딩과 에러, 빈 상태도 처리한다.
   리포트의 `errors[].code`와 `feTodo`를 빠짐없이 화면에 반영한다.
7. 공용화 판단. 스킬 §3의 게이트 A/B/C를 쓴다. 데이터와 타입, fetch 규약은 굳으면 공용, 화면 표현은 앱별,
   순수 원자와 유틸은 두 번째 쓰이면 공용이다. 첫 API는 앱별로 붙이고 두 번째에서 같은 shape면 그때 승격한다.
   애매하면 앱별로 두고 후보로만 기록한다.
8. 검증은 둘로 갈린다. 계약과 DB(응답 shape가 매핑과 맞는지, 뮤테이션이 DB에 반영되는지, 에러 코드)는
...
9. 커밋과 PR 글. 티켓 키를 넣고 `pnpm format` 후 커밋, push. push 전에 `./scripts/review/pr-review.sh`를 돌리고
   CRITICAL이 있으면 먼저 고친다. PR은 올리지 않고 제목과 본문을 채팅에 쓴다.
...
- be-verify 리포트의 `blocked.reason`이 "BE 안 떠 있음"이면 BE를 대신 띄우지 않는다. 사용자에게 알리고 기다린다.
- 리포트의 `cleanup.remaining`이 0이 아니면 테스트 유저가 남은 것이다. be-verify에 정리를 다시 시키고 0을 확인한 뒤 진행한다.
- 스웨거와 실제가 다르고(`swaggerDiffs`) 어느 쪽이 맞는지 판단이 안 서면 사용자에게 BE 담당과 확인해 달라고 한다. 실제 응답 기준으로 임시 매핑하고 TODO에 남긴다.
- 빌드나 타입 검사가 실패하면 원인을 고친다. `any`나 타입 단언으로 덮지 않는다.
