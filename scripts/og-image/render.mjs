/**
 * PLick OG 기본 이미지(1200x630) 생성 스크립트.
 * 브랜드 토큰(다크 bg, accent)과 @plick/ui Logo 워드마크 벡터를 그대로 써서
 * 정적 PNG를 만든다. 태그라인 텍스트는 Pretendard SemiBold로 렌더한다.
 */
import { Resvg } from "@resvg/resvg-js";
import { writeFileSync } from "node:fs";

const BG = "#0b0d12";
const TEXT = "#eef2f8";
const TEXT_3 = "#93a0b4";
const ACCENT = "#2fd97f";

const TAGLINE = "프리미어리그 소식을 릴스로";

/* Logo.tsx 벡터 원본 (viewBox 0 0 33.0001 10.5383) */
const WORDMARK_PATHS = `
  <path d="M8.22051 10.4064V0.766837H10.6916V8.40602H14.6453V10.4064H8.22051Z" fill="${TEXT}"/>
  <path d="M0 10.4065V0.766865H4.0667C4.78842 0.766865 5.41442 0.90807 5.94473 1.19048C6.47503 1.46975 6.88609 1.86356 7.17792 2.37189C7.46974 2.87709 7.61565 3.46545 7.61565 4.13696C7.61565 4.81474 7.4666 5.40466 7.1685 5.90672C6.8704 6.40879 6.44836 6.79632 5.90237 7.06931C5.35951 7.34231 4.72095 7.47881 3.98669 7.47881H1.5156V5.5443H3.50659C3.83607 5.5443 4.11377 5.48782 4.3397 5.37485C4.56876 5.25875 4.74292 5.09558 4.86216 4.88534C4.98453 4.67197 5.04572 4.4225 5.04572 4.13696C5.04572 3.85141 4.98453 3.60508 4.86216 3.39798C4.74292 3.19088 4.56876 3.03242 4.3397 2.92259C4.11377 2.80963 3.83607 2.75315 3.50659 2.75315H2.47109V10.4065H0Z" fill="${TEXT}"/>
  <path d="M15.0911 10.3466V3.25727H17.5819V10.3466H15.0911Z" fill="${TEXT}"/>
  <path d="M27.912 8.599L27.9261 5.68548H28.2556L30.1477 3.17673H32.9106L29.9312 6.92337H29.2581L27.912 8.599ZM25.7092 10.4064V0.766837H28.1567V10.4064H25.7092ZM30.1807 10.4064L28.4109 7.47407L30.0112 5.73725L33.0001 10.4064H30.1807Z" fill="${TEXT}"/>
  <path d="M21.614 10.5383C20.8389 10.5383 20.1753 10.3829 19.623 10.0723C19.0739 9.75851 18.6518 9.32234 18.3569 8.7638C18.0619 8.20212 17.9144 7.55101 17.9144 6.81046C17.9144 6.07306 18.0619 5.42352 18.3569 4.86183C18.6518 4.30015 19.0754 3.86399 19.6277 3.55333C20.18 3.23955 20.8421 3.08265 21.614 3.08265C22.3012 3.08265 22.899 3.20817 23.4073 3.4592C23.9156 3.70709 24.311 4.05854 24.5934 4.51353C24.8758 4.96538 25.0217 5.49569 25.0311 6.10444H22.753C22.7122 5.73103 22.5946 5.44234 22.4 5.23838C22.2086 5.03128 21.9576 4.92773 21.6469 4.92773C21.4022 4.92773 21.1872 4.99833 21.0021 5.13954C20.817 5.2776 20.6726 5.48471 20.5691 5.76084C20.4655 6.03697 20.4137 6.38057 20.4137 6.79164C20.4137 7.2027 20.4639 7.54787 20.5644 7.82714C20.6679 8.10327 20.8123 8.31194 20.9974 8.45315C21.1825 8.59121 21.399 8.66025 21.6469 8.66025C21.8478 8.66025 22.0266 8.61632 22.1835 8.52846C22.3404 8.43746 22.4675 8.3041 22.5648 8.12838C22.6652 7.95265 22.7279 7.73771 22.753 7.48354H25.0311C25.0186 8.1017 24.8727 8.63985 24.5934 9.09798C24.3141 9.55611 23.9219 9.9107 23.4167 10.1617C22.9115 10.4128 22.3106 10.5383 21.614 10.5383Z" fill="${TEXT}"/>
  <path d="M17.5394 1.24401C17.5394 1.93106 16.9825 2.48802 16.2954 2.48802C15.6084 2.48802 15.0514 1.93106 15.0514 1.24401C15.0514 0.556962 15.6084 0 16.2954 0C16.9825 0 17.5394 0.556962 17.5394 1.24401Z" fill="${ACCENT}"/>
`;

/* 워드마크 배치: 폭 520px, 가로 중앙, 세로는 태그라인과 묶어 광학 중앙보다 살짝 위 */
const LOGO_W = 520;
const SCALE = LOGO_W / 33.0001;
const LOGO_H = 10.5383 * SCALE;
const LOGO_X = (1200 - LOGO_W) / 2;
const LOGO_Y = 214;

const svg = `<svg width="1200" height="630" viewBox="0 0 1200 630" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <radialGradient id="glow" cx="50%" cy="38%" r="65%">
      <stop offset="0%" stop-color="${ACCENT}" stop-opacity="0.10"/>
      <stop offset="55%" stop-color="${ACCENT}" stop-opacity="0.03"/>
      <stop offset="100%" stop-color="${ACCENT}" stop-opacity="0"/>
    </radialGradient>
  </defs>
  <rect width="1200" height="630" fill="${BG}"/>
  <rect width="1200" height="630" fill="url(#glow)"/>
  <g transform="translate(${LOGO_X} ${LOGO_Y}) scale(${SCALE})">${WORDMARK_PATHS}</g>
  <text x="600" y="${LOGO_Y + LOGO_H + 78}" text-anchor="middle" font-family="Pretendard" font-size="42" fill="${TEXT_3}">${TAGLINE}</text>
</svg>`;

const resvg = new Resvg(svg, {
  fitTo: { mode: "width", value: 1200 },
  font: {
    fontFiles: ["./Pretendard-SemiBold.otf"],
    loadSystemFonts: false,
    defaultFontFamily: "Pretendard",
  },
});
writeFileSync("opengraph-image.png", resvg.render().asPng());
console.log("done: opengraph-image.png");
