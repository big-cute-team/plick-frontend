 ...
 * ({@link ReporterListButton})으로 바뀐다 — 이름 오른쪽 위에 기자 수가 붙고
 * 눌러서 기자 목록을 펼친다(표시 전용, 원문 이동은 원문 버튼 몫 — KAN-365).
 * 한 명이면 표기 없이 이름만 나온다. 기자 배열은 기사 상세 응답에만 있어
 * 릴 세부는 상세를 받기 전까지 안 주는데, 그동안도 피드가 준 대표 한 명이
 * 이름으로만 나온다.
 *
 * @param reporter - 표시할 대표 기자(name·tier). 펼침 버튼이 아닐 때의 표시.
 * @param reporters - 기사에 실린 기자 전원(중복 제거·대표 순서). 두 명 이상이면
 *   펼침 버튼.
