...
const ITEMS: { question: string; answer: ReactNode }[] = [
  ...
  },
  {
    ...
    answer: (
      <p>
        ...
        도움이 되도록 소식을 보도한 기자를 함께 표시하고 원문 링크를 엽니다.
      </p>
    ),
  },
  {
    ...
    answer: (
      <p>
        둘러보기는 로그인 없이 가능합니다. 댓글 작성과 응원팀 설정은 로그인이
        필요하며, 로그인은 카카오, 구글, 애플 소셜 로그인만 지원합니다.
      </p>
    ),
  },
  {
    ...
    answer: (
      <p>
        MY &gt; 계정에서 언제든지 바꿀 수 있습니다. 여러 팀을 함께 선택할 수도
        있습니다.
      </p>
    ),
  },
  {
    ...
    answer: (
      <p>
        MY 화면 맨 아래의 회원 탈퇴로 바로 탈퇴할 수 있습니다. 탈퇴하면 계정을
        복구할 수 없고, 소셜 연결·이메일·닉네임·응원팀 정보가 삭제됩니다. 작성한
        댓글은 &apos;탈퇴한사용자&apos;가 쓴 것으로 표시된 채 남습니다. 같은
        소셜 계정으로는 탈퇴 후 7일이 지나야 다시 가입할 수 있으며, 다시
...
/**
 * FAQ 본문 (KAN-372). 질문과 답을 항상 펼친 채로 나열한다. KAN-567에서 카드 섀시를
 * 걷고 목록 선으로 가른다(시안 전역 규칙: 본문 카드에 테두리 없음).
 *
  ...
  return (
    <div>
      {ITEMS.map((item) => (
        <section
          key={item.question}
          className="border-border-soft flex flex-col gap-1.5 border-b py-3.5"
        >
          <h2 className="text-body-md text-text-strong font-bold">
            {item.question}
          </h2>
