...
/**
 * LIVE 탭 — 방송 전파 형태 (KAN-435). 라이브 스코어 탭이 기사 탭 자리를
 * 대체하면서 신문(NewsIcon)을 걷어내고 넣었다. 피그마 시안 없이 하단 탭
 * 세트(Home·Reels·User)의 기하(24 그리드·1.8 스트로크)에 맞춰 직접 그렸다.
 * 가운데 점만 면이라 fill로 채운다.
 */
...
/**
 * VS 탭 — 교차한 쌍검(전투) 모티프. 사용자가 준 레퍼런스(칼몸·가드·그립·폼멜이
 * 전부 외곽선 면으로 그려진 쌍검)를 24 그리드·currentColor로 따라 그렸다.
 * 앞검(오른쪽 위 칼끝)은 통짜, 뒷검 날은 앞검이 지나가는 자리를 끊어 앞뒤를
 * 구분한다. 스트로크는 세트 기본 1.8 대신 1.2다 — 면 도형들의 흰 속이 1.8에서는
 * 메워져 실루엣만 남는다. 머리 둘 + VS 레터폼 버전을 대체한다.
 */
export const VsIcon = (p: IconProps) => (
  <svg {...base(p)} strokeWidth={1.2}>
    <path d="M2.8 2.8 5.03 3.69 11.26 9.92 9.92 11.26 3.69 5.03 Z" />
    <path d="M14.08 12.74 16.77 15.43 15.43 16.77 12.74 14.08 Z" />
    <path d="M14.51 17.77 15.93 19.19 19.19 15.93 17.77 14.51 Z" />
    <path d="M17.4 18.6 19.05 20.25 20.25 19.05 18.6 17.4 Z" />
    <path d="M18.98 20.82 20.18 22.02 22.02 20.18 20.82 18.98 Z" />
    <path d="M21.2 2.8 20.31 5.03 8.57 16.77 7.23 15.43 18.97 3.69 Z" />
    <path d="M9.49 17.77 8.07 19.19 4.81 15.93 6.23 14.51 Z" />
    <path d="M6.6 18.6 4.95 20.25 3.75 19.05 5.4 17.4 Z" />
    <path d="M5.02 20.82 3.82 22.02 1.98 20.18 3.18 18.98 Z" />
  </svg>
);
...
export const FileTextIcon = ({ size = 19, ...p }: IconProps) => (
  <svg {...base({ size, ...p })}>
...
/**
 * 애플 로고 심볼 (KAN-395). Sign in with Apple HIG 요구사항이라 실제 애플 마크를 그대로 쓴다.
 * 색은 `currentColor`로 버튼 글자색(on-apple = 흰색)을 따른다.
 *
 * @param size - 렌더 크기(px)
 */
export const AppleIcon = ({ size = 19, ...p }: IconProps) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
  ...
  >
    <path
      d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.47-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.08zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"
      fill="currentColor"
    />
  </svg>
);
