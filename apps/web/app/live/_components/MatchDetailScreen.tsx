...
/**
 * 상태로 좌측 본문이 갈린다 — SCHEDULED는 프리뷰·뉴스, LIVE·FINISHED는
 * 요약·라인업·스탯·뉴스, POSTPONED는 프리뷰가 있으면 프리뷰, 없으면 안내만,
 * CANCELLED는 안내만(`MATCH_TABS_BY_STATUS`). 우측 채팅 패널은 상태와 무관하게
 * 늘 같은 자리에 있어 탭을 바꿔도 리마운트되지 않는다.
 *
 * 탭 줄은 탭이 둘 이상일 때만 선다. 뉴스가 붙기 전(KAN-484)에는 예정 경기가
 * 프리뷰 하나뿐이라 그 상태에서만 줄이 없었는데, 지금은 탭이 없는 상태가
