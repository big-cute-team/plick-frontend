...
/**
 * 요약 탭의 득점 카드(피그마 LW4 → KAN-462 확대) — BE 득점 요약(시간순, 취소 골 제외)을
...
export function GoalsCard({ goals }: { goals: MatchGoal[] }) {
  ...
  return (
    <section className="bg-elevate rounded-card flex flex-col gap-3 p-6">
      <h2 className="text-title text-text font-bold">득점</h2>
      {goals.map((goal, i) => (
        <p
          key={`${goal.minute}-${i}`}
          className={`flex items-center gap-2.5 ${goal.side === "AWAY" ? "justify-end" : ""}`}
        >
          <span aria-hidden className="bg-accent size-2 rounded-full" />
          <span className="text-body text-text-3 font-bold">{goal.minute}</span>
          <span className="text-body-lg text-text font-semibold">
            {goal.playerName}
          </span>
          {goal.detail && (
            <span className="text-body text-text-4">({goal.detail})</span>
          )}
        </p>
      ))}
    </section>
