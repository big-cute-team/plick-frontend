import { SendMiniIcon, UserRoundIcon } from "@plick/ui/icons";

/**
 * 댓글 입력바 — pill 인풋 + accent 원형 전송 버튼(+ 선택 아바타).
 * 기사 세부·릴 세부 패널 공용. 전송은 BE 연동 시 핸들러를 붙인다.
 *
 * @param withAvatar - 앞에 사용자 아바타 원을 표시(릴 세부 패널)
  ...
  return (
    <form className={`flex items-center gap-2.5 ${className}`}>
      ...
      <input
        ...
        className="bg-elevate-2 border-border text-body text-text placeholder:text-text-4 focus-visible:border-border-strong rounded-pill h-11 min-w-0 flex-1 border px-4 focus-visible:outline-none"
      />
      <button
        type="submit"
        aria-label="댓글 등록"
