...
/**
 * 데스크톱 상단 GNB — 로고 + 내비게이션.
 *
 * 검색 인풋과 알림·프로필 아이콘은 대응 기능이 없어 걷어냈다(KAN-338).
 * 테마 토글도 걷어냈다 — 웹·모바일 모두 다크 고정으로 돌렸다. 라이트 토큰은
  ...
  return (
    <header className="bg-nav border-border sticky top-0 z-40 border-b">
      <PageContainer className="flex h-16 items-center gap-4 lg:gap-7">
        ...
        {/* 모바일: 내비를 숨기고 이 스페이서로 햄버거를 우측으로 민다 */}
        <div aria-hidden className="flex-1" />

        <MobileNav className="shrink-0 lg:hidden" />
      </PageContainer>
    </header>
  );
}
