...
/**
 * 로그인 여부·내 닉네임을 클라 컴포넌트로 흘려주는 컨텍스트 (모바일 `AuthProvider` 복제, KAN-318).
 *
 ...
 *
 * 닉네임은 내 댓글 판별(KAN-333 수정·삭제 버튼)에 쓴다 — 댓글 목록 응답에 작성자 식별 필드가
 * 없어 닉네임 대조가 유일한 수단이다(온보딩·프로필 수정에서 BE가 중복을 거절하므로 앱 수준에선
 * 유일하고, 오판해도 BE가 403으로 막는다).
 *
 ...
 * @param nickname 내 닉네임. 비로그인·온보딩 전·프로필 조회 실패면 null —
...
export function AuthProvider({
  ...
  return (
    <AuthContext.Provider value={{ isLoggedIn, nickname }}>
      {children}
 ...
 *
 * @returns `{ isLoggedIn, nickname }` — 유저 정보가 더 붙으면 이 객체를 넓힌다.
 */
export function useAuth(): { isLoggedIn: boolean; nickname: string | null } {
