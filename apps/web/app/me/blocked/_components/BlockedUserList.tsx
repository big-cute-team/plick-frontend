...
/**
 * 차단 목록 (KAN-411, 모바일 `BlockedUserList` 복제 → KAN-567 시안 MY 721-735행).
 * 서버가 준 목록을 초깃값으로 들고, 해제하면 그 줄을 로컬 state에서 뺀다. 이 화면
 * 밖에서 목록이 바뀔 일이 없고(차단은 댓글에서만 늘고, 이 화면은 서버 컴포넌트라
 * 재진입마다 새로 받는다) 해제는 완전 멱등이라 RQ 캐시 없이 로컬 상태로 충분하다.
 *
 * 행은 닉네임 14/700, "YYYY.MM.DD 차단" 12 보조색, 오른쪽 "차단 해제" 12.5/700 텍스트
 * 버튼(hover 빨강)이다. 아바타 이니셜은 시안 규칙(프로필 이미지 없음)대로 지웠다.
 * 해제는 티켓대로 확인 팝업 없이 바로 보낸다. 멱등이라 오클릭 비용이 낮고, 잘못
 * 눌렀으면 그 유저 댓글에서 다시 차단하면 된다. 댓글 캐시 정리(원문 복원을 위한
 * refetch 유도)는 `useUnblockUser`가 한다.
 *
...
export function BlockedUserList({ initial }: { initial: BlockedUser[] }) {
  ...
  return (
    <>
          ...
          >
            <div className="flex min-w-0 flex-1 flex-col gap-0.75">
              ...
              <span className="text-label text-text-3">
                {formatBlockedAt(user.blockedAt)} 차단
              </span>
            </div>
            <button
              type="button"
              ...
              disabled={isPending}
              className="text-label-lg text-text-3 hover:text-danger focus-visible:outline-accent shrink-0 font-bold focus-visible:outline-2 focus-visible:outline-offset-2 disabled:opacity-40"
            >
      ...
      {error && (
        <p role="status" className="text-caption text-danger mt-2.5">
          {error}
        </p>
      )}
...
/**
 * 차단 시각을 시안 표기 "2026.09.14"로. 기기 시간대와 무관하게 KST로 고정한다
 * (`formatDateKo`와 같은 이유). `ko-KR` 숫자 포맷은 "2026. 09. 14."라 공백과 끝 점을 걷는다.
 *
 * @param iso 차단 시각 ISO-8601
 */
