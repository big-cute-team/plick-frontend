/**
 * @file 마이페이지 활동 화면 타입 (KAN-495 모바일 → KAN-567 웹 이식). 모바일
 * `_types/activity.ts`의 복제다. 두 앱이 같은 계약을 쓰게 됐으니 다음 정리 때
 * `@plick/domain/types`로 승격할 후보다(ADR 0011 게이트 C). 좋아요한 기사는 홈 피드
 * 카드와 같은 모양이라 새 타입 없이 `ArticleCard`, `ArticleFeedPage`를 그대로 쓴다.
 */

import type { ArticleFeedPage } from "@plick/domain/types";

/**
 * 활동 탭. URL `?tab=`의 값이자 쿼리키의 스코프다. 시안(KAN-567 MY 575-739행) 순서는
 * 내 댓글, 좋아요, 내 투표인데 투표 목록 API가 없어 `votes`는 빈 상태만 그린다.
 */
...
/**
 * 활동 화면 서버 컴포넌트가 미리 받아 클라 캐시에 심을 씨앗.
 *
