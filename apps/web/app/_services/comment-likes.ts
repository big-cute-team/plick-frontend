 ...
 * (KAN-309, web 이식 KAN-331, `POST`·`DELETE /api/v1/comments/{commentId}/like`).
 *
 * 보호 API라 HttpOnly 쿠키의 access 토큰을 Bearer로 실어야 하는데 클라는 그
 * 쿠키를 못 읽으므로 서버 액션을 경유한다. 기사 좋아요(`article-likes.ts`)와
 * 같은 이유·같은 모양이고, 응답 shape(`{ likeCount }`)도 같다. 쿠키 이름이
 * 앱에 박혀 있어 승격하지 않고 모바일 `_services/comment-likes.ts`를 복제했다.
 *
