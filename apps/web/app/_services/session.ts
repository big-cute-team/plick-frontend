 ...
 * 모바일 `_services/session.ts` 복제다(KAN-318, `getAccessToken`은 기사 세부가 첫
 * 소비처가 된 KAN-322에서 따라왔다).
 */
...
/**
 * 서버 렌더 중 access 토큰을 꺼낸다 — 읽기 fetch에 Bearer로 실을 때 쓴다(KAN-322).
 *
 * 기사 상세는 토큰 없이도 200이 오는 공개 API지만, 토큰이 있으면 응답의
