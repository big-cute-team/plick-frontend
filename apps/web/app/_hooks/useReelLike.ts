...
/**
 * 릴 좋아요 토글 (KAN-308, web 이식 KAN-330) — {@link useArticleLike}에 릴스판
 * 원본 갱신을 물린다.
 *
 * 릴 카드의 원본은 무한 쿼리 캐시다. 컴포넌트 state로 들면 스크롤로 벗어났다
 * 돌아왔을 때 캐시의 옛 값으로 되돌아가므로, 낙관적 갱신도 롤백도 캐시를 직접
 * 고친다. 다음 리페치가 오면 서버 값이 그대로 덮는다 — 그게 맞는 값이다.
 *
