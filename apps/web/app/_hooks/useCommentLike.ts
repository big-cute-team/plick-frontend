...
/**
 * 댓글 좋아요 토글 (KAN-309, web 이식 KAN-331) — {@link useLikeToggle}에 댓글판
 * 원본 갱신을 물린다. 훅은 승격 후보 목록에 없어 모바일 `useCommentLike`를 복제했다.
 *
 * 댓글의 원본은 목록 무한 쿼리 캐시 하나뿐이다. 기사 세부는 서버가 첫 페이지를
 * 씨앗으로 심고 릴 세부 패널은 클라에서 처음 받지만, 둘 다 같은 쿼리키를 보므로
 * 캐시만 고치면 두 화면 모두 맞는다. 컴포넌트 state로 들면 패널을 닫았다 열거나
