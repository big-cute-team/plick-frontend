 ...
 * 모바일 `_hooks/useReelUrlSync.ts`와 동일 구현이다 — 훅은 앱별로 둔다.
 *
 ...
 * 않으므로 릴이 리렌더 없이 주소만 바뀐다. `router.replace`는 소프트 내비게이션이라
 * 딥링크 라우트를 실제로 마운트해 피드가 통째로 갈린다.
 *
 * state는 버리지 않고 그대로 되넘겨 Next 내부 히스토리 state를 보존한다.
