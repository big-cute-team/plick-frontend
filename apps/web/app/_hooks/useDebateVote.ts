...
/**
 * 토론 투표 뮤테이션 (KAN-418) — `useLikeToggle`과 같은 뼈대다. 모바일
 * `useDebateVote`의 웹 이식이다 — 서버 액션·AuthProvider가 앱 로컬이라 훅째
 * 복제한다(댓글 훅과 같은 판단).
 *
 * 상태를 이 훅이 들지 않고 `onChange`로 호출부에 되돌려주는 이유도 같다 —
 * 원본이 서버 props → 컴포넌트 state에 있다. 여기에 더해 같은 토론이 토론
 * 리스트·릴스 피드 캐시에도 살아 있어, 갱신·확정·롤백 때마다 `syncDebateVote`로
 * 캐시까지 함께 맞춘다.
 *
