...
/**
 * 인물 관련 기사 무한 목록 (KAN-500, web 이식 KAN-501). 모바일
 * `_hooks/useFigureArticles.ts`와 동일 구현이다 — 훅은 승격 후보 목록에 없어
 * 앱별로 둔다(`useArticleFeed`와 같은 판단).
 *
 * 팀 피드 훅과 같은 규약이다 — 서버가 받은 첫 페이지를 씨앗으로 심고,
 * `nextCursor`가 null이면 마지막 페이지, 4xx는 재시도하지 않는다. 인물마다
 * 쿼리키가 달라 다른 인물로 넘어가면 새로 받고, 봤던 인물로 돌아오면 신선한
 * 동안 캐시를 그대로 쓴다.
 *
