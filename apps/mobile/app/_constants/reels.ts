...
/** 릴 섹션 대비 시트 높이 비율 — 시안(KAN-567) 86%. 전에는 피그마 352/480.7(0.73)이었다 */
export const SHEET_HEIGHT_RATIO = 0.86;

/**
 * 정보 블록(팀·제목·기자 줄) 하단과 시트 상단 라인 사이 간격(px). 시트가 86%로 커지면서
 * (KAN-567) 도킹 자리가 상단 14%뿐이라 피그마 값 18에서 10으로 줄였다. 블록이
 * 섹션 위로 잘리지 않게 하는 여유다.
 */
export const SHEET_TITLE_GAP = 10;
