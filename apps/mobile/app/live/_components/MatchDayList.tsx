 ...
 * 카드를 쌓는다(피그마 L1). 대회 등장 순서는 데이터 순서를 따른다.
 */
export function MatchDayList({ matches }: { matches: MatchSummary[] }) {
  const groups: { competition: string; matches: MatchSummary[] }[] = [];
  for (const match of matches) {
    const last = groups[groups.length - 1];
    if (last?.competition === match.competition) last.matches.push(match);
    else groups.push({ competition: match.competition, matches: [match] });
  }

  return (
    <div className="px-edge flex flex-col gap-3 pt-2">
      {groups.map((group) => (
        <section key={group.competition} className="flex flex-col gap-2">
