...
import type { Filter, RumorStage, TeamCode } from "@plick/domain/types";
import { apiFetch } from "@/_apis/client";
import { TEAM_CODES, TEAM_IDS } from "@/_constants/api";
import type { ArticleCard, ArticleFeedPage } from "@/_types/articles";
...
/**
 * BE 단계 값 → 도메인 `RumorStage`.
 * BE는 `RUMOR`, 도메인 타입은 영국식 `RUMOUR`라 철자가 한 글자 다르다.
 * 모르는 값이 새로 생기면 배지를 지우는 편이 잘못된 배지보다 낫다.
 */
