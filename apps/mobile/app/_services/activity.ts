/**
 * @file 마이페이지 활동 조회 fetcher (KAN-495, `GET /api/v1/users/me/likes`·
 * `/users/me/comments`·`/users/me/activity`).
 *
 * 서버 컴포넌트(활동 화면·마이페이지의 첫 페이지와 개수)와 클라 훅(다음 페이지·
 * 새로고침) 양쪽에서 부르므로 서버 액션이 아니라 평범한 모듈이다. base URL 선택은
 ...
 * 데이터가 공유 캐시에 남지 않는다.
 *
 * 첫 사용처가 모바일뿐이라 앱 레이어에 둔다. web이 두 번째 사용처가 되면
 * `@plick/core`로 승격한다(ADR 0011 게이트 C).
 */
...
import { ACTIVITY_PAGE_SIZE } from "@/_constants/activity";
import type {
  MyActivityCounts,
  MyComment,
  MyCommentPage,
...
/** 커서·건수를 쿼리스트링으로, 토큰을 헤더로 만든다. 세 fetcher가 같은 모양이다. */
