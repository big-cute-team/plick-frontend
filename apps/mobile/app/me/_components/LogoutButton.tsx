...
/**
 * 로그아웃 텍스트 버튼 + 확인 시트. MY 맨 아래의 로그아웃 진입점.
 *
 * KAN-567 시안대로 채운 버튼이 아니라 12.5 회색 텍스트 버튼이고, 확인은 팝업이
 * 아니라 바텀 시트(`ConfirmDialog`)다. 누르면 "정말 로그아웃할까요?" 시트를 띄우고
 * (오탭 방지), 확인해야 로그아웃 서버 액션을 부른다. 액션이 도는 동안
 * `useTransition`의 `isPending`으로 시트 버튼을 잠가 연타를 막는다. 성공하면
 * 액션이 서버에서 홈(`/`)으로 redirect하므로 여기선 닫는 처리조차 필요 없다.
 *
