...
/**
 * 카카오/구글/애플 버튼 묶음. OAuth 시작 서버 액션을 연결하는 클라 경계. 높이는
 * KAN-567 시안의 확정 버튼 48이다.
  ...
  return (
    <>
      <SocialLoginButton
        ...
        className="bg-kakao text-on-kakao h-12"
        disabled={isPending}
      ...
      <SocialLoginButton
        ...
        className="bg-media-on text-on-kakao border-border-strong h-12 border"
        disabled={isPending}
      ...
      <SocialLoginButton
        ...
        className="bg-apple text-on-apple h-12"
        disabled={isPending}
        onClick={() => loginWith("APPLE")}
      />
      {error && (
        <p className="text-caption-lg text-danger text-center" role="alert">
          {error}
        </p>
      )}
    </>
