...
- **세션 시작 시**: 새 ADR 파일 하나 생성 → `docs/adr/000N-<짧은-설명>.md`(N = 다음 번호). 그 세션에서 **무엇을·어떻게·왜 했고 어디서 막혔는지**를 담는다("무엇"보다 **어떻게·왜·막힌 지점** 중심).
- **세션 중 변경이 생기면**: 새 파일을 또 만들지 말고 **그 세션 ADR에 계속 이어 붙인다**(후속 피드백·버그 수정도 같은 md의 절로). 같은 PR/브랜치 작업은 같은 ADR.
- **문체 — 포트폴리오 블로그 회고체로 쓴다.** ADR은 사내 규격 문서가 아니라 나중에 블로그·포트폴리오에 올릴 **내 회고 글**이라고 생각하고 쓴다.
  - **1인칭 회고 흐름**: "이걸 만들었다 → 여기서 이런 문제를 만났다 → 이렇게 풀었다"를 자연스럽게 이어 쓴다.
  - **짧고 쉽게**: 한 편을 몇 분 안에 읽을 수 있게. 표·소제목·불릿 나열은 최소화하고 **문단 위주**로. 곁가지는 과감히 잘라낸다.
  - **기술 메커니즘은 파고들어 설명한다(중요).** 서버 액션·쿠키·내비게이션(soft/hard·prefetch)·캐싱·서버/클라 경계·리다이렉트처럼 **"어떻게/왜 그렇게 되는가"가 걸리는 대목**은 얼버무리지 말고 **요청/응답 흐름, 무엇이 어디서 도는지, 왜 그 방식인지**까지 풀어 쓴다. 목표는 읽는 사람(=나)이 나중에 따로 되물을 필요가 없는 것. `"use server"`=서버 액션(≠단순 서버 실행), `cookies()`=요청 헤더 읽기·`Set-Cookie`로 지시, `redirect()`=소프트 내비게이션(리로드 아님) 같은 **오해하기 쉬운 지점은 정의부터 바로잡아** 준다. ⚠️ 위 "짧게"는 **곁가지(잡일·삽질 나열)를 자르라는 뜻이지 메커니즘 설명을 생략하라는 게 아니다** — 이 둘은 충돌하지 않는다.
  - **사람이 쓴 티**: 담백한 `~했다`체, 막힌 데는 막혔다고 솔직하게. **AI 티 나는 말투 금지** — 과장 형용사(`강력한`·`매끄러운`·seamless·robust류), "결론적으로"·"~할 수 있습니다" 남발, 기계적 나열, 문단마다 소제목 다는 습관.
