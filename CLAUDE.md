...
예: `@/_lib/types`, `@/_components/AppShell`, `@/reels/_lib/constants` (라우트 그룹은 `@/(home)/_lib/…`)
...
컴포넌트 파일에 타입이나 상수, 순수함수를 인라인으로 두지 않는다. 역할별 파일로 나눠 `_lib`에 둔다.
2개 이상 화면이 쓰면 `app/_lib/`, 한 라우트 전용이면 그 라우트의 `_lib/`에 둔다(예: `app/reels/_lib/`).

- `types.ts`: 타입과 인터페이스. 여러 컴포넌트가 공유하거나 중복되는 타입은 반드시 여기로.
- `constants.ts`: 상수와 설정값.
- `utils.ts`: 순수 헬퍼 함수.
- `mock.ts`: 목데이터(나중에 fetch로 교체될 가짜 데이터). 상수와 구분한다.
- 훅은 파일 하나에 훅 하나로 `useXxx.ts`.

⚠️ web과 mobile이 함께 쓰는 도메인 타입(FeedPost 등)과 참조 상수(TEAMS 등), 포맷 유틸은
`@plick/domain`(types/constants/format)이 단일 출처다(ADR 0018). 앱 `_lib`에는 앱 전용만 남긴다
